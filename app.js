...
io.on('connection', function(socket) {
  ...
  client.on('qr', (qr) => {
    ...
    qrcode.toDataURL(qr, (err, url) => {
      socket.emit('qr', url);
      socket.emit('message', 'Kode QR diterima, tolong pindai!');
    });
  });
  ...
  client.on('auth_failure', function(session) {
    socket.emit('message', 'Kegagalan autentikasi, mulai ulang...');
  });
  ...
  //   console.log('Sent message:', msg.body);
  //   socket.emit('message', 'Sent message :'+ msg.body);
  //   // You can implement your own logic here to handle incoming messages
  //   // For example, you can emit a socket event or send a response back to the client
  // });
  client.on('message_create', (msg) => {
    // Handle incoming messages
    const messageBody = msg.body;
    // const recipientNumber = msg.to._serialized;
    socket.emit('message', 'Pesan Masuk/Keluar: '+ messageBody);
    // You can implement your own logic here to handle incoming messages
    // For example, you can emit a socket event or send a response back to the client
  });
  // client.on('message', (msg) => {
  ...
  //   console.log('Sent message:', msg.body);
  //   socket.emit('message', 'Sent message :', msg.body);
  //   // You can implement your own logic here to handle sent messages
  //   // For example, you can emit a socket event or send a response back to the client
  // });
